    ...
    if os.path.exists(CVS_FILE):
        ...
        for cv in cvs:
            ...
            if 'name' not in cv:
                ...
                cv['name'] = str(cv['name'])  # Ensure the name is a string
        return cvs
    else:
        return []
...
def parse_cv(cv_path):
    try:
        ...
        score, found_skills, years = calculate_score(text)
...
def main():
    ...
    if choice == "Home":
        ...
        all_names = [str(cv['name']) for cv in st.session_state.cvs]  # Ensure all names are strings
        ...
        if selected_name:
            filtered_cvs = [cv for cv in st.session_state.cvs if selected_name == str(cv['name'])]
        else:
            filtered_cvs = [cv for cv in st.session_state.cvs if search_name.lower() in str(cv['name']).lower()]
